    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex justify-end">
      <div className="bg-slate-900 border-l border-slate-800 w-full max-w-md h-full flex flex-col justify-between p-4 sm:p-5 shadow-2xl animate-in slide-in-from-right">
        ...
        <div className="flex-1 overflow-y-auto py-3 space-y-2.5">
          {items.length === 0 ? <p className="text-center text-xs text-slate-500 py-10">Keranjang belanja Anda masih kosong.</p> : items.map((i) => (
            <div key={i.product.id} className="bg-slate-850 p-2.5 rounded-xl border border-slate-800 flex items-center justify-between gap-2">
              <div className="min-w-0 flex-1"><h5 className="text-xs font-bold text-slate-200 truncate">{i.product.name}</h5><div className="text-[11px] text-emerald-400 font-mono font-semibold">{formatRupiah(i.product.promoPrice)}</div></div>
        ...
        <div className="pt-3 border-t border-slate-800 space-y-2.5 text-xs">
          ...
          <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Alamat Pengiriman Lengkap (Jalan, RT/RW, Patokan)..." className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-emerald-500" />
          ...
          {discount > 0 && <div className="flex justify-between text-emerald-400"><span>Kupon ({appliedVoucher?.code}):</span><strong className="font-mono">-{formatRupiah(discount)}</strong></div>}
          ...
          <button onClick={handleCheckoutWA} disabled={items.length === 0} className="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 disabled:opacity-50 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-950/60 active:scale-95 transition-all"><Send className="w-4 h-4" /><span>Kirim Pesanan via WhatsApp (COD)</span></button>
        </div>
      </div>
    </div>
  );
